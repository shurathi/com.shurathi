 ...
 * PROGRAM DESCRIPTION:                                           
 *                                                                
 * Delinquency Calculation - This constant file is created for calculate trigger events. 
 *                                                             
 ********************************************************************************************
 *                                                                
 * CHANGE HISTORY:                                                
...
public class CmDelinquencyProcessCalculateTriggerDateConstants {
	...
	public final static StringBuilder LATEST_DUE_DATE_SQL = new StringBuilder("from CmDelinquencyProcessRelatedObject dpo, Bill bl ")
			.append(" where dpo.id.delinquencyProcess.id = :delinProcId ")
			.append(" and dpo.id.maintenanceObject = 'BILL' ")
			.append(" and dpo.id.cmDelinquencyRelatedObjTypeFlg = :relatedObjTypeFlag ")
			.append(" and dpo.id.primaryKeyValue1 = bl.id ");

	public final static StringBuilder CALENDER_WORK_DAY_QRY = new StringBuilder("FROM FeatureConfigurationOption WO ")
			.append(" WHERE WO.id.workforceManagementSystem = :deliqProcOptFeatureConfig AND WO.id.optionType = :deliqProcOptionType");

	public final static StringBuilder STATE_OF_ISSUE_CUSTOMER_LVL = new StringBuilder(" SELECT DISTINCT PPC.SRCH_CHAR_VAL AS STATE_OF_ISSUE ")
	...
	public final static StringBuilder GRACE_DAYS_DIVISION = new StringBuilder(" SELECT MAX(TO_NUMBER(DIVC.ADHOC_CHAR_VAL)) AS GRACE_DAYS, ")
	...
	public final static StringBuilder DRAG_DAYS_CHAR = new StringBuilder(" SELECT MAX(TO_NUMBER(AC.ADHOC_CHAR_VAL)) DRAG_DAYS ")
	...
	public final static StringBuilder CUST_DIVISION = new StringBuilder(" from AccountPerson ap, ")
			.append(" Account ac ")
			.append(" where ac.id = ap.id.account.id ")
			.append(" and ap.id.person = :personId ")
			.append(" and ap.isMainCustomer = 'Y' ");

	public final static StringBuilder CUST_BROKER_CHK_CUST = new StringBuilder(" from PersonId perId , CmBrokerInformation bi")
			.append(" where perId.id = :personId ")
			.append(" and perId.isPrimaryId = 'Y' ")
			.append(" and bi.id.cmSourceCustomerId = perId.personIdNumber ");

	public final static StringBuilder CUST_BROKER_CHK_ACCT = new StringBuilder(" from AccountPerson ap , PersonId perId , CmBrokerInformation bi")
	...
	public final static StringBuilder ACCT_INV_DAY_CHAR = new StringBuilder(" from AccountCharacteristic ac, Account a, ")
	...
	public final static StringBuilder COLL_CLS_CUST = new StringBuilder(" from CmPersonCollection pc, ")
			.append(" AccountPerson ap ")
	...
	public final static StringBuilder FETCH_NEXT_BILL_DATE = new StringBuilder("from BillCycleSchedule bcs where bcs.id.billCycle = :billCycle ")
