 ...
 * PROGRAM DESCRIPTION:
 *
 * This algorithm will extract an email for Courtesy Notice
 *
 ***********************************************************************
 *
 * CHANGE HISTORY:
 ...
 */
public class CmCourtesyNoticeAlgComp_Impl extends CmCourtesyNoticeAlgComp_Gen
		implements CollectionActionAlgorithmSpot {
	...
	private static final String OUT_MSG_ID = "outboundMessageId";
	private static final String SEND_DETAIL = "sendDetail";
	...
	public void invoke() {
		...
		if(!isNull(accountId.getEntity())){
			msg = msg.replace("%1", accountId.getIdValue());			
		}else{
		...
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittanceContact"))){
			msg = msg.replace("%2",remittanceDetailGroup.getString("remittanceContact"));			
		}else{
		...
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittanceName"))){
			msg = msg.replace("%3", remittanceDetailGroup.getString("remittanceName"));			
		}else{
			addError(CmMessageRepository.requiredAttributeNotFoundInExtendableLookup("Remittance Name",extendableLookup.entityName(), person.getId().getTrimmedValue()));
		}
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittanceAddress1"))){
			msg = msg.replace("%4",remittanceDetailGroup.getString("remittanceAddress1"));			
		}else{
			addError(CmMessageRepository.requiredAttributeNotFoundInExtendableLookup("Remittance Address",extendableLookup.entityName(), person.getId().getTrimmedValue()));
		}
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittanceCity"))){
			msg = msg.replace("%5",remittanceDetailGroup.getString("remittanceCity"));			
		}else{
			addError(CmMessageRepository.requiredAttributeNotFoundInExtendableLookup("Remittance City",extendableLookup.entityName(), person.getId().getTrimmedValue()));
		}
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittanceState"))){
			msg = msg.replace("%6",remittanceDetailGroup.getString("remittanceState"));
		}else{
			addError(CmMessageRepository.requiredAttributeNotFoundInExtendableLookup("Remittance State",extendableLookup.entityName(), person.getId().getTrimmedValue()));
		}
		if(!isBlankOrNull(remittanceDetailGroup.getString("remittancePostal"))){
			msg = msg.replace("%7",remittanceDetailGroup.getString("remittancePostal"));			
		}else{
			addError(CmMessageRepository.requiredAttributeNotFoundInExtendableLookup("Remittance Address",extendableLookup.entityName(), person.getId().getTrimmedValue()));
		}
