 ...
 */
public class CmCreateToDoForDelinquencyAlgComp_Impl extends CmCreateToDoForDelinquencyAlgComp_Gen implements BusinessObjectEnterStatusAlgorithmSpot {
	...
	public void invoke() {
		...
		// Retrieve the Delinquency Process Person or Account the flattened elements
		personId = new Person_Id(boInstance.getString(CmCreateToDoForDelinquencyConstant.PERSON_ID));
		...
		if ((isNull(personId) && isNull(accountId)) || (notNull(personId) && notNull(accountId))) {
		...
		if (notNull(accountId)) {
			...
			// Retrieve the Accounts main customers primary name and
			// collection class
			Query<QueryResultRow> query = createQuery(CmCreateToDoForDelinquencyConstant.RET_PRIM_NAME_AND_COLL_CLASS.toString(), "CmCreateToDoForDelinquencyAlgComp_Impl");
			query.bindId("accountId", accountId);
			query.addResult("primaryName", "PN.entityName");
			query.addResult("collectionClass", "PC.collectionClass.id");
			row = query.firstRow();

			if (notNull(row)) {
				customerName = row.getString("primaryName");
				collClassId = (CollectionClass_Id) row.getId("collectionClass", CollectionClass.class);
		...
		if (notNull(personId)) {
			...
			if (notNull(personId.getEntity())) {
				// Retrieve the persons primary name and store as customerName
				customerName = personId.getEntity().getPersonPrimaryName();

				// Retrieve the persons collection class and store as
				// collectionClass
			...
			// Create To Do Entry
			ToDoEntry_Id toDoEntryId = createToDoEntry(delinquencyProcess_Id, businessObjectStatusCd, customerName, collClassId.getIdValue());
